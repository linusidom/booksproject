...
class Post(models.Model):
	...
	def ensure_profile_exists(sender, **kwargs):
		if kwargs.get('created', False):
			...
			Post.objects.get_or_create(user=kwargs.get('instance'), chapter = 2, question='The turning point in the lives of those who succeed usually comes at the moment of some crisis, through which they are introduced to their other selves.  What is the turning point in your life?')
			...
			Post.objects.get_or_create(user=kwargs.get('instance'), chapter = 5, question='Those who are not successful usually make the mistake of believing that the knowledge acquiring period ends when one finishes school.  How have you continued to acquire specialized knowledge?')
			...
			Post.objects.get_or_create(user=kwargs.get('instance'), chapter = 9, question='If you find yourself lacking in persistence, this weakness may be remedies by building a stronger fire under your desires.  How can building a stronger fire under your desires help to overcome a lack of persistence?')
			...
			Post.objects.get_or_create(user=kwargs.get('instance'), chapter = 12, question='Why do negative emotions enter our thought impulses voluntarily while positive ones must be injected?')
			...
			Post.objects.get_or_create(user=kwargs.get('instance'), chapter = 14, question='How might an imaginary Council aid you in obtaining your goal or desire?')
